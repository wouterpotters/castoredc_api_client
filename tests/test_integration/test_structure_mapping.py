...
class TestStudyMap:
    ...
    def test_map_study_number(self, integration_study, client):
        """Tests if the right number of forms, steps and fields are linked."""
        integration_study.map_structure(client)
    ...
    def test_map_study_ids(self, integration_study, client):
    ...
    def test_study_get_all_survey_forms(self, integration_study, client):
    ...
    def test_study_get_all_report_forms(self, integration_study, client):
    ...
    def test_study_form_links_model(self, integration_study, client):
        """Tests if the links are in the right format."""
        integration_study.map_structure(client)
        integration_study.update_links(client)
    ...
    def test_study_form_links_form(self, integration_study, client):
        """Tests if the links contain the right forms."""
        integration_study.map_structure(client)
        integration_study.update_links(client)
        assert "89FF2394-0D41-4D4C-89FE-AA9AB287B31E" in integration_study.form_links["Report"]
        assert "C4ADC387-9BFD-4171-A861-6B973699A6ED" in integration_study.form_links["Report"]
        assert "770DB401-6100-4CF5-A95F-3402B55EAC48" in integration_study.form_links["Report"]
        assert "8D4F696E-DA2E-4A0E-ACD8-2F1B71718D6E" in integration_study.form_links["Report"]
        assert "D70C1273-B5D8-45CD-BFE8-A0BA75C44B7E" in integration_study.form_links["Survey"]

    def test_study_form_links_form_instances(self, integration_study, client):
        ...
        assert (
            ...
            in integration_study.form_links["Report"]["89FF2394-0D41-4D4C-89FE-AA9AB287B31E"]
        )
        assert (
            "98CB10D0-B392-4EB9-BEEF-BCBCC59D9A6F"
            in integration_study.form_links["Report"]["C4ADC387-9BFD-4171-A861-6B973699A6ED"]
        )
        assert (
            "C8EE71C5-F266-4F59-BFD3-A643643C4FE1"
            in integration_study.form_links["Report"]["770DB401-6100-4CF5-A95F-3402B55EAC48"]
        )
        assert (
            "2924E308-5718-48D6-881B-7492F350B8F7"
            in integration_study.form_links["Report"]["8D4F696E-DA2E-4A0E-ACD8-2F1B71718D6E"]
        )
        # Test Surveys
        assert (
            "QOL Survey"
            in integration_study.form_links["Survey"]["D70C1273-B5D8-45CD-BFE8-A0BA75C44B7E"]
        )

    def test_map_study_data(self, integration_study, client):
        """Tests if the right data is added to the objects"""
        # Test a field
        integration_study.map_structure(client)
        field = integration_study.get_single_field("75497076-FDA2-415F-88E5-5BD51D50A8D1")
    ...
    def test_map_study_link_field_step(self, integration_study, client):
    ...
    def test_map_study_link_step_form(self, integration_study, client):
